...
* `/codegen/model/:modelName/props`: возвращает полный список полей, имеющихся у модели, в виде списка, разделенного запятой
* `/codegen/model/:modelName/inputs`: возвращает список полей для форм ввода, имеющихся у модели, в виде списка, разделенного запятой
* `/codegen/model/:modelName/inputs/code`: возвращает код для полей для форм ввода, имеющихся у модели
* `/codegen/model/:modelName/filter`: возвращает список полей для формы фильтра, имеющихся у модели, в виде списка, разделенного запятой
* `/codegen/model/:modelName/filter/code`: возвращает код для полей для формы фильтра, имеющихся у модели
* `/codegen/model/:modelName/fields`: возвращает список полей для форм списков, имеющихся у модели, в виде списка, разделенного запятой
* `/codegen/model/:modelName/input/:propName`: возвращает код для поля формы ввода для свойства`propName`
* `/codegen/model/:modelName/field/:propName`: возвращает код для поля формы списка для свойства `propName`

Автоматически генерируемые сниппеты: 
* `app-model-{{nameKebab}}-filter-fields`: код для полей фильтра
* `app-model-{{nameKebab}}-list-fields`
* `app-model-{{nameKebab}}-form-fields`
* `app-model-{{nameKebab}}-enum-defs`
...
* функция `app.exModular.services.seed(modelName, fileName, opt)`:
...
По умолчанию, загрузка данных поизводится из папки `SEEDS_DIR` из файла `.env`. Существует возможность добавить
"варианты" данных путем указания имени вложенной в указанную директорию папки, для этого указываем свойство 
`app.exModular.seedVariantFolder`. Тогда система будет приоритетно 
...
* **примечание**: метод `app.exModular.storages.Clear()` очищает все таблицы и приводит к загрузке данных заново
