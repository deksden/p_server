...
(!) Партионный учёт поддерживается следующим образом:
* схема содержит поля batchId, dateProd, dateExp; также добавляем к вендору expDuration - если не 
задана dateExp в приходной партии, можем сделать эту дату; 
...
* каждая расходная операция сопоставляется с партией; 

указывать сроки годности партии ресурсов как справочную информацию - и указывать какое сырье с каким сроком
годности планируется использовать на каком этапе; указывать партии сырья, номера серий, сроки производства и
сроки годности для партий;

Подумать над внесением информации о сроках годности ресурсов. Установить базовые сроки годности в информации о каждом 
поставщике ресурса. В остатках при списании указывать - какую партию товара списываем.

Партионный учёт ресурсов: каждый приход образует определенную партию с идентификатором этого поступления; это будет
идентификатор партии. Все расходные операции должны делаться с указанием - из какой партии брали ресурс. Делаем доп
поля batchId (партия ресурса), batchDatePr (дата производства), batchDateExp (дата окончания срока годности), 
batchPrice (цена партии), batchVendor (поставщик партии).  
